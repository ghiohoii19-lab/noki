...
// Handler untuk gerakan pointer (mouse/touch/stylus)
...
function renderOverlay() {
  ...
  if (isResizingSelection) {
    let newW = Math.max(1, w + previewOffset.dx);
    let newH = Math.max(1, h + previewOffset.dy);
    ctx.drawImage(selectionBuffer,
      x, y,
      newW, newH
    );
  ...
  showTransformHandles(selection); // kotak + handle
}
...
function drawHandles(ctx) {
  ...
  ctx.save();
  ctx.setTransform(1,0,0,1,0,0); // reset transform supaya tidak ikut zoom
  ctx.fillStyle = "white";
  ctx.strokeStyle = "black";

  corners.forEach(c => {
    // transform koordinat ke layar
...
canvas.addEventListener("pointermove", handleRulerMove);
...
canvasContainer.addEventListener("touchstart", e => {
  if (e.touches.length === 2) {
    ...
    initialTouches = { distance, centerX, centerY };
  }
}, { passive: false });
...
canvasContainer.addEventListener('touchmove', (e) => {
            if (isDrawing) {
                draw(e.touches[0]);
                render();
            } else if (e.touches.length === 2 && initialTouches !== null) {
                e.preventDefault();
                const touch1 = e.touches[0];
                const touch2 = e.touches[1];
                
                const currentDx = touch1.clientX - touch2.clientX;
                const currentDy = touch1.clientY - touch2.clientY;
                ...
                let newScale = scale * (currentDistance / initialTouches.distance);

                if (newScale >= MIN_SCALE && newScale <= MAX_SCALE) {
                    scale = newScale;
                }

                const dx = currentCenterX - initialTouches.centerX;
                const dy = currentCenterY - initialTouches.centerY;

                panX += dx / scale;  // <--- PERBAIKAN: Dibagi dengan scale
                panY += dy / scale;
                
                initialTouches.distance = currentDistance;
                initialTouches.centerX = currentCenterX;
                initialTouches.centerY = currentCenterY;
...
canvas.addEventListener("touchend", e => {
  ...
  // kalau dua kali tap dalam 300ms → undo
  if (tapInterval > 0 && tapInterval < 300) {
    undo(); // panggil fungsi undo kamu
  }
